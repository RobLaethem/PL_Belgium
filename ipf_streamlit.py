...
with st.sidebar:
    ...
    st.write(
        f"The percentile of a {lift} of {total_input} kg in the {selected_weight_class} kg class for the year {selected_year} is: {percentile:.2f}%")
...
n = len(df_class)
# calculate mean and median of the lift column
mean = df_class[lift_selection].mean()
median = df_class[lift_selection].median()
# calculate 10% and 90% of the lift column (rank-based %)
...
# calculate the min and max of the lift column
...
# create a plot containing a histogram of the lift column showing how many lifters lifted a certain total weight
